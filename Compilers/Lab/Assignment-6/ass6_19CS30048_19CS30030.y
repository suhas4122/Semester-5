...
%{
    ...
    // yylex for lexer, yyerror for error recovery
...
%union{
    ...
    string *strval;     // to hold the value of a constant
    decStr decl;        // to define the declarators
    arglistStr argsl;   // to define the arguments list
...
// Tokens
%token BREAK CASE CHAR CONST CONTINUE DEFAULT DO DOUBLE ELSE EXTERN
%token FLOAT FOR GOTO IF INLINE INT LONG RESTRICT RETURN SHORT SIZEOF STATIC STRUCT SWITCH
%token TYPEDEF UNION VOID VOLATILE WHILE 
...
postfix_expression :
                    ...
                    postfix_expression '(' argument_expression_list_opt ')' 
                    {
                        ...
                        if($3.arguments == NULL)
                        {
                            // When there are no function Parameters
                            sprintf(str,"0");
                        ...
                        {
                            // When there are function parameters
                            if((*$3.arguments)[0]->isString)
                            {
                                // If the prameter is a string
                                str[0] = '_';
...
unary_expression:   postfix_expression 
                    {
                        // Just equate to expression
                        $$ = $1;
                    }|
                    INCREMENT unary_expression 
                    {
                        // Generate new temporary, equate it to original value
                    ...
                    unary_operator cast_expression
                    {
                        ...
                        switch($1)
                        {
                            ...
                            case '!':
                                $$.loc = curr_st->gentemp(CopyType($2.type));
                                $$.type = $$.loc->tp_n;
                                $$.truelist = $2.falselist;     // Make truelist
                                $$.falselist = $2.truelist;     // Make falselist
                                break;
                            default:
                                break;
                        }
...
cast_expression :   unary_expression 
                    {
                        ...
                        {
                            // Dereferencing as it is a pointer
                            $$.loc = curr_st->gentemp(CopyType($1.type));
                            $$.isPointer = false;
...
multiplicative_expression:  cast_expression 
                            ...
                            multiplicative_expression '*' cast_expression 
                            {
                                ...
                                typecheck(&$1,&$3);
                                // Generate new temporary if the types are compatible
...
shift_expression:   additive_expression 
                    {
                        // Just equate the expression
                        $$ = $1;
                    }|
                    shift_expression LEFT_SHIFT additive_expression 
                    {
                        // Left shift for an integer value
                        $$.loc = curr_st->gentemp($1.type);
                        $$.type = $$.loc->tp_n;
                    ...
                    {
                        // Right shift for an integer value
                        $$.loc = curr_st->gentemp($1.type);
                        $$.type = $$.loc->tp_n;
...
relational_expression:  shift_expression 
                        ...
                        relational_expression '<' shift_expression 
                        {
                            ...
                            $$.truelist = makelist(next_instr);             // Make truelist
                            $$.falselist = makelist(next_instr+1);          // Make falselist
                        ...
                        relational_expression LESS_EQUALS shift_expression 
                        {
                            ...
                            typecheck(&$1,&$3);
                            $$.type = new type_n(tp_bool);
                            $$.truelist = makelist(next_instr);             // Making truelist
                            $$.falselist = makelist(next_instr+1);          // Making falselist
...
logical_AND_expression:     inclusive_OR_expression 
                            ...
                            {
                                // Convert int to bool and backpatch
                                ...
                                $$.type = new type_n(tp_bool);
                                $$.falselist = merge($1.falselist,$4.falselist);        // Make falselist
                                $$.truelist = $4.truelist;          // Make truelist
                            };
...
logical_OR_expression:      logical_AND_expression 
                            ...
                            {
                                // Convert to bool if required and backpatch
                                if($1.type->basetp != tp_bool)
                                    conv2Bool(&$1);
                                if($4.type->basetp != tp_bool)
                                    conv2Bool(&$4); 
                                ...
                                $$.type = new type_n(tp_bool);
                                $$.truelist = merge($1.truelist,$4.truelist);       // Make truelist
                                $$.falselist = $4.falselist;                        // Make falselist
                            };
...
conditional_expression:     logical_OR_expression 
                            ...
                            {
                                // Generate new temporary to store the expression
                                ...
                                // Merge the next_instr list
                                TEMP_LIST = merge(TEMP_LIST,makelist(next_instr));
                                glob_quad.emit(Q_GOTO,"-1");
                                // Backpatching with respect to the OR expression
...
assignment_expression:          
                        ...
                        {
                            //Dereferencing if type is pointer
                            if($1.isPointer)
                            {
...
init_declarator:    declarator 
                    ...
                    declarator '=' initializer 
                    {
                        ...
                        if($3.type!=NULL)
                        {
                        if($3.type->basetp==tp_int)     // Integer type
                        {
                        ...
                        else if($3.type->basetp==tp_char)       // character type
                        {
...
type_specifier:     VOID { glob_type = new type_n(tp_void);}|
                    CHAR { glob_type = new type_n(tp_char);}|
                    SHORT {}|
                    INT { glob_type = new type_n(tp_int);}|
                    LONG {}|
                    FLOAT {}|
                    DOUBLE { glob_type = new type_n(tp_double);} ;
...
block_item_list:                block_item { $$ = $1; }|
                                ...
                                {
                                    // move to block_item via $2 after $1
                                    backpatch($1,$2);
                                    $$ = $3;
                                };
...
selection_statement:            IF '(' expression N ')' M statement N ELSE M statement 
                                {
                                    /*N1 is used for falselist of expression, 
                                    M1 is used for truelist of expression, 
                                    N2 is used to prevent fall through, 
                                    M2 is used for falselist of expression
                                    */
                                    $7 = merge($7,$8);
                                ...
                                {
                                    /*N is used for the falselist of expression to skip the block 
                                    and M is used for truelist of expression*/
...
iteration_statement:            WHILE '(' M expression N ')' M statement 
                                {
                                    /*The first 'M' takes into consideration that the control will come again at the beginning of the condition checking.
                                    'N' here does the work of breaking condition i.e. 
                                    it generate goto which will be useful when we are exiting from while loop. 
                                    Finally, the last 'M' is here to note the starting statement that will be executed in every loop to populate the truelists of expression
                                    */
                                ...
                                {  
                                    /*M1 is used for coming back again to the statement 
                                    as it stores the instruction which will be needed by the truelist of expression. 
                                    M2 is needed as we have to again to check the condition which will be used to populate the nextlist of statements. 
                                    Further N is used to prevent from fall through
                                    */
                                    backpatch($8,next_instr);
                                    backpatch($3,$6);           // S1.nextlist to M2.instr 
                                    conv2Bool(&$7);             // Convert expression to bool
                                    backpatch($7.truelist,$2);  // B.truelist to M1.instr
                                    $$ = $7.falselist;
                                }|
                                FOR '(' expression_opt ';' M expression_opt N ';' M expression_opt N ')' M statement 
                                {
                                    /*M1 is used for coming back to check the epression at every iteration. 
                                    N1 is used  for generating the goto which will be used for exit conditions. 
                                    M2 is used for nextlist of statement and N2 is used for jump to check the expression 
                                    and M3 is used for the truelist of expression
                                    */
                                    ...
                                    conv2Bool(&$6);              // Convert expression to bool
...
jump_statement:                 GOTO IDENTIFIER ';' {}|
                                ...
                                RETURN expression_opt ';' 
                                {
                                    ...
                                    {
                                        // Creating a new dummy expression
